...
while ret:
    ...
    if detections is None:
        print(f"No detections for frame {frame_count}")
        ret, frame = vid.read()
        frame_count += 1
        continue
    ...
    if detections is not None and len(detections) > 0:
        ...
        for det in detections:
            ...
            cv2.putText(frame, c, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, colour, 2)

            # Add the detection to the csv
            rows.append([frame_count, c, x1, y1, box_w, box_h])
